...
int eval(brd *br)
{
    ...
    // if(pc_cnt < 5){ // just for 3-4 piece endgames
    //     int to_add = (abs(index_to_pos(op_king).x - index_to_pos(my_king).x)) * 50;
    //     if(ans > 0){
    //         to_add = 7 - to_add;
    //     }
    //     ans += to_add;
...
unsigned long long int zbr_table[64][12];
unsigned long long int zbr_table_spec[10];
void init_table_h(){
    unsigned int seed = time(0);
    for(int i = 0; i < 64; ++i){
        for(int k = 0; k < 12; ++k){
            zbr_table[i][k] = rand_r(&seed);
        }
    }
    for(int i = 0; i < 10; ++i){
        zbr_table_spec[i] = rand_r(&seed);
    }
...
unsigned long long int zbr_hash(brd *br){
    ...
    h^=zbr_table_spec[2]*br->LB_castle;
    h^=zbr_table_spec[3]*br->LW_castle;
    h^=zbr_table_spec[4]*br->RB_castle;
    h^=zbr_table_spec[5]*br->RW_castle;
    return h;
}
